...
std::string CodeTracer::CODEITEM_IN	="IN";
std::string CodeTracer::CODEITEM_OUT	="OUT";
...
void CodeTracer::initialize(int iArgc, char* argv[])
{
	...
	for ( int i=1; i<iArgc;i++)
	{
		...
		{
			std::string sLeftValue = sArg.substr(0, iPos);
			std::string sRightValue = sArg.substr(iPos+1);
...
void CodeTracer::createTraceFile()
{
	...
	std::string sTracePath = System::getEnvironmentVariable(CM_TRACEPATH);
...
void CodeTracer::write(short level, CodeItem* item, int iCodeLineNumber, int iItemAction)
{
	...
	sTrc.append("-");
	/*
	std::string sThreadId = StringUtils::toString((int)Thread::getCurrentThreadId());
...
std::string CodeTracer::indentbyThread(int iThreadId, int iItemAction)
{
