...
Semaphore::Semaphore( std::string Name, int initialCount, int maxCount )
{
        ...
        if (Name == "")
        {
            ...
            if ( nResult != 0 )
            {
                std::string msg = "Semaphore non created. " + std::string( strerror(errno) );
        ...
        else
        {
            ...
            std::string s = "/" + Name;
            m_hSemaphore = *sem_open( s.c_str(), O_CREAT, S_IRUSR | S_IWUSR | S_IXUSR, 0 );
            if ( (int)&m_hSemaphore == -1 )
            {
	            std::string msg = "Semaphore non created. " + std::string( strerror(errno) );
                throw SynchronizationException( msg );
            }
...
Semaphore::~Semaphore()
{
        ...
        else
        {
            ...
            if( callingThread == _OwnerThread )
            {
                ...
                if ( nResult == -1)
                {
                    std::string msg = "Semaphore Unlink Failed. " + std::string( strerror(errno) );
                    throw SynchronizationException( msg );
                }
            }
        }
...
void Semaphore::post()
{
        ...
        if ( nResult != 0 )
        {
	        std::string msg = "Semaphore post failed. " + std::string( strerror(errno) );
      	    throw SynchronizationException( msg );
        }
    #endif
}
