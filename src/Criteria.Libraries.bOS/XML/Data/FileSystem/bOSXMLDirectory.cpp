...
std::string XMLTable::MAIN_NODE		= "Table";
std::string XMLTable::SEQUENCE_NODE	= "Sequence";

std::string XMLDirectory::FILENAME	= "Table.xml";
...
XMLNode* XMLTable::getXMLNode(unsigned int iHow)
{
	...
	node->addNode( new XMLNode( XMLTable::SEQUENCE_NODE.c_str(), lSequence) );
	return node;
}
...
long XMLDirectory::insert(XMLFile* obj, const std::string& sFileName, bool bAutoIncrement)
{
	long lIdSequence=0;
...
void XMLDirectory::get(XMLFile* obj, const std::string& sFileName)
{
	std::string sFile=sPath;
	sFile+="\\";
	sFile+=sFileName;

	if ( sFileName.find(".xml") == - 1)
	{
		sFile+=".xml";
	}
