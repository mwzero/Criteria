...
std::string vSpecialCode[SPECIAL_CODE_NUM] = {"&lt;","&gt;", "&amp;"};
*/
char vSpecialCharacter[SPECIAL_CODE_NUM] = {'&'};
std::string vSpecialCode[SPECIAL_CODE_NUM] = {"&amp;"};
...
XMLNode::XMLNode( const char* acName, const std::string& _Value)
{
    sName = acName;
    sValue = _Value;
    nodeParent = (XMLNode*)NULL;
...
XMLNode::XMLNode( const char* acName, long lValue)
{
    sName = acName;
...
std::string XMLNode::getValueTranscoded(const char* acValue)
{
...
std::string XMLNode::getXMLText( bool bIndent, unsigned int uilevel )
{
    std::string sSeparator = NEWLINE;
    std::string sIndentator = "";

    if ( bIndent )
	{
		sIndentator=std::string(uilevel, '\t');
	}
	else
	{
        sSeparator = "";
...
std::string XMLNode::getInnerXML()
{
	StringBuffer sb;
