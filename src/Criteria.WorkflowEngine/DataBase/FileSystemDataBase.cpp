...
String FileSystemDataBase::getForm(const char* acFormName)
{
	...
	try
	{
		if ( (dbObj == NULL) || dbObj->isFileRepository() )  {
			...
			for ( unsigned int i=0; i<files.size(); i++) {
				...
				bOS::IO::File file2Add(file2Read.getData());
				_DataTypes.addFile(file2Add);
...
void FileSystemDataBase::getDataType(DataType& _DataType)
{
	...
	*/
	...
	//TODO: Aggiungere la possibilità di rileggerlo da files.
	if ( datatype == NULL )
		throw DBException(DB_RECORD_NOT_FOUND);
	...
	try
	{
		if ( (dbObj == NULL) || dbObj->isFileRepository() )  {
			...
			for ( unsigned int i=0; i<files.size(); i++) {
				...
				bOS::IO::File file2Add(file2Read.getData());
				_Applications.addFile(file2Add);
...
DataManager::Panorama::WorkflowProcesses::WorkflowProcess* FileSystemDataBase::getProcessTemplate(const char* acWorkflowName)
{
	...
	try
	{
		CM_TRACE_CODEPATH("Retrieving File[" << sWorkflowFile.getData() << "]")
