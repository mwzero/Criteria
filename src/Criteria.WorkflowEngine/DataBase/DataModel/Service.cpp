...
void Service::setRecord(ResultSetRecord& record)
{
	...
	record.add ( new ResultSetField (SQLService::USERS_FIELD.c_str(), vUsers.getXMLString(1).c_str()));
	record.add ( new ResultSetField (SQLService::ROLES_FIELD.c_str(), vRoles.getXMLString(1).c_str()));
	record.add ( new ResultSetField (SQLService::EXTENDEDATTRIBUTES_FIELD.c_str(), vExtendedAttributes.getXMLString(1).c_str()));
}
