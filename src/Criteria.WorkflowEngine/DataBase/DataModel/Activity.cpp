...
bOS::XML::XMLNode* Activity::getXMLNode(unsigned int iHow)
{
	...
	case CATCHER:
		...
		break;
    case NONE:
        sType = XMLActivity::NONE_TYPE;
        break;
    case ITERATION:
        sType = XMLActivity::ITERATION_TYPE;
        break;
	}
...
bool Activity::equalTo(bOS::Collections::CollectionEntry* entry)
{
	...
	CM_TRACE_CODEPATH(
                   "Activity ProcessId [" << activity->getProcessId() << "] "
                   << "this ProcessId [" << this->getProcessId() << "] "
                   << "Name [" << activity->getActivityName() << "] "
                   << "this Name [" << this->getActivityName() << "] ")
...
void Activity::setRecord(ResultSetRecord& record)
{
	...
	//
	record.add( new ResultSetField (SQLActivity::GRANTS_FIELD.c_str(), vGrants.getXMLString(0).c_str() ));
	record.add( new ResultSetField (SQLActivity::BEHAVIOUR_FIELD.c_str(), behaviour.getXMLString(0).c_str() ));
	record.add( new ResultSetField (SQLActivity::LOG_FIELD.c_str(), log.getXMLString(0).c_str() ));
	record.add( new ResultSetField (SQLActivity::LOGS_FIELD.c_str(), vLogs.getXMLString(0).c_str() ));
	record.add( new ResultSetField (SQLActivity::REPORTS_FIELD.c_str(), vReports.getXMLString(0).c_str() ));
