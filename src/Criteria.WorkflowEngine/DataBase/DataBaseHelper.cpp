...
#include "bOS/bOSFileSystemException.h"
...
#include "DataModel/Connections.h"
#include "DataModel/Connection.h"
#include "DataModel/mySQLConnection.h"
#include "DataModel/ODBCConnection.h"
#include "DataModel/SQLiteConnection.h"
#include "DataModel/FileSystemConnection.h"
...
void DataBaseHelper::open(String sConfigurationFile)
{
	...
	for ( unsigned int i=0; i<uiSize; i++)
	{
		...
		if ( connection->getIdType() == 1 )//mySQL
		{
			...
			bOS::SQL::mySQL::mySQLConnectionData* mySQLconnectionData = new bOS::SQL::mySQL::mySQLConnectionData();
			DataManager::Panorama::DataBase::mySQLConnection* myConnection = (DataManager::Panorama::DataBase::mySQLConnection*)connection;

			CM_TRACE_INFO("ConnectionData to mySQL. Host[" << myConnection->getHost() << "] Port["
                 << myConnection->getPort() << "]")

			mySQLconnectionData->sHost = myConnection->getHost();
			CM_TRACE_INFO("ConnectionData to mySQL. Host[" << mySQLconnectionData->sHost << "]")
...
String DataBaseHelper::getForm(const char* acFormName)
{
	for ( unsigned int i=0; i<vDataBases.size();i++)
	{
		DataBaseTemplate* db = vDataBases.at(i);
...
String DataBaseHelper::createProcess(const char* acWorkflowName, WorkflowProcess& process, bool b2Prepared)
{
	...
	CM_TRACE_CODEPATH("Process [" << acWorkflowName << "] retrieved")
	...
	CM_TRACE_CODEPATH("Process Instance. ID[" << process.getProcessId() << "]")
...
String DataBaseHelper::getProcessStatus(const char* processId)
{
