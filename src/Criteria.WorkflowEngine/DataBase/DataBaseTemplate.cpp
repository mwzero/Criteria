...
void DataBaseTemplate::getActivities(Activities& vActivities, String sStatus, String sOwner, bool bOnlyMine)
{
	...
	if ( bCheckGrant )
	{
		...
		this->executeRecursiveSQLCommands (NULL, &activities, bOS::SQL::SQLCommand::SQLOperation::SELECT, sWhere.getData(), false );
	...
	{
		this->executeRecursiveSQLCommands (NULL, &vActivities, bOS::SQL::SQLCommand::SQLOperation::SELECT, sWhere.getData(), false );
	}
}
...
void DataBaseTemplate::getNotifies(Activities& vActivities, String sOwner)
{
	...
	this->executeSQL (NULL, &vActivities, bOS::SQL::SQLCommand::SQLOperation::SELECT, sSQL.getData(), false );
}
...
void DataBaseTemplate::insertProcess(String& lProcessId, WorkflowProcess& process,bool b2Prepared )
{
	...
	try
	{
		...
		this->executeSQLCommands (connection, &process, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);
		CM_TRACE_CODEPATH("insert attributes");
		this->executeSQLCommands (connection, &process.getAttributes(),bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, false);
		...
		for ( unsigned int i=0; i<uiSize; i++)
		{
			...
			this->executeSQLCommands (connection, activity, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, false);
			CM_TRACE_CODEPATH("insert activity attributes");
			this->executeSQLCommands (connection, &(activity->getAttributes()), bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, false);
		}
	}
	catch ( ... )
	{
		...
		{
			process.setError();
			this->executeSQLCommands (connection, &process, bOS::SQL::SQLCommand::SQLOperation::MODIFY, NULL, true);
		}
		throw;
	}
...
void DataBaseTemplate::executeRecursiveSQLCommands(SQLConnection* connection, SQLTable* sqlTable, bOS::SQL::SQLCommand::SQLOperation operation, const char* acWhere, bool bMainTable)
{
...
int DataBaseTemplate::executeSQLCommands(SQLConnection* connection, SQLTable* table, bOS::SQL::SQLCommand::SQLOperation iOperation, const char* acWhere, bool bMainTable)
{
	...
	switch ( iOperation )
	{
	case bOS::SQL::SQLCommand::SQLOperation::ADD:
		table->getInsertSQLCommand(vCommands);
		break;
	case bOS::SQL::SQLCommand::SQLOperation::MODIFY:
		table->getUpdateSQLCommand(vCommands);
		break;
	case bOS::SQL::SQLCommand::SQLOperation::ERASE:
		table->getDeleteSQLCommand(vCommands);
		break;
	case bOS::SQL::SQLCommand::SQLOperation::SELECT:
		table->getSelectSQLCommand(vCommands);
		bReturnRecordSet = true;
		break;
	case bOS::SQL::SQLCommand::SQLOperation::CREATE:
		table->getCreateSQLCommand(vCommands);
		break;
	case bOS::SQL::SQLCommand::SQLOperation::DROP:
	...
	try
	{
		...
		for ( int i=0;i<iSize;i++)
		{
			...
			else
			{
				...
				if ( ( iOperation == bOS::SQL::SQLCommand::SQLOperation::ADD)  && ( table->haveAutoIncrementField() ) )
				{
...
int DataBaseTemplate::executePrepareSQLCommands(SQLConnection* connection, SQLTable* table, bOS::SQL::SQLCommand::SQLOperation iOperation, const char* acWhere, bool bMainTable)
{
...
int DataBaseTemplate::executeSQL(SQLConnection* connection, SQLTable* table, bOS::SQL::SQLCommand::SQLOperation iOperation, const char* acSQL, bool bMainTable)
{
...
void DataBaseTemplate::getUser(User& user)
{
	...
	executeRecursiveSQLCommands(NULL, &user, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
}
...
void DataBaseTemplate::addUser(const char* acUser)
{
	...
	this->executeRecursiveSQLCommands ( NULL, &user, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);
}
...
void DataBaseTemplate::updateUser(const char* acUser)
{
	...
	this->executeRecursiveSQLCommands ( NULL, &user, bOS::SQL::SQLCommand::SQLOperation::MODIFY, NULL, true);
}
...
void DataBaseTemplate::deleteUser(const char* acUserKey)
{
	...
	this->executeRecursiveSQLCommands ( NULL, &user, bOS::SQL::SQLCommand::SQLOperation::ERASE, NULL, true);
}
...
void DataBaseTemplate::getRole(DataManager::Panorama::WorkflowParticipantsData::Role& role)
{
	...
	this->executeRecursiveSQLCommands(NULL, &role, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
}
...
void DataBaseTemplate::addRole(const char* acRole)
{
	...
	this->executeRecursiveSQLCommands(NULL, &role, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);
}
...
void DataBaseTemplate::updateRole(const char* acRole)
{
	...
	this->executeRecursiveSQLCommands(NULL, &role, bOS::SQL::SQLCommand::SQLOperation::MODIFY, NULL, true);
}
...
void DataBaseTemplate::deleteRole(const char* acRoleKey)
{
	...
	this->executeRecursiveSQLCommands(NULL, &role, bOS::SQL::SQLCommand::SQLOperation::ERASE, NULL, true);
}
...
void DataBaseTemplate::getDataType(DataManager::Panorama::WorkflowDataTypes::DataType& datatype)
{
	...
	this->executeRecursiveSQLCommands(NULL, &datatype, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
}
...
void DataBaseTemplate::insertReport(WorkflowProcesses::Report& report)
{
	...
	this->executeRecursiveSQLCommands(NULL, &report, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);
}
...
void DataBaseTemplate::getReports (WorkflowProcesses::Reports& vReports)
{
	...
	this->executeRecursiveSQLCommands (NULL, &vReports, bOS::SQL::SQLCommand::SQLOperation::SELECT, sWhere.getData(), false );
}
...
void DataBaseTemplate::addService(const char* acService)
{
	...
	this->executeRecursiveSQLCommands(NULL, &service, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);
}
...
void DataBaseTemplate::deleteService(const char* acServiceName)
{
	...
	this->executeRecursiveSQLCommands(NULL, &service, bOS::SQL::SQLCommand::SQLOperation::ERASE, NULL, true);
}
...
String DataBaseTemplate::getProcessStatus(const char* processId)
{
...
void DataBaseTemplate::getProcessHeader(WorkflowProcess& process)
{
	...
	this->executeSQLCommands (connection, &process, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
}
...
void DataBaseTemplate::getProcessInstance(WorkflowProcess& process)
{
	...
	//this->executeRecursiveSQLCommands(NULL, &process, SELECT, NULL, true);
	SQLConnection* connection = NULL;
	try
	{
		connection = manager.get(iRetry, iDelay);
	...
	this->executeSQLCommands (connection, &process, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
	this->executeSQLCommands (connection, &(process.getAttributes()), bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, false);

	WorkflowProcesses::Activities& vActivities = process.getActivities();
	this->executeSQLCommands (connection, &vActivities, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, false);
	unsigned int uiSize = vActivities.size();
	for ( unsigned int i=0; i<uiSize; i++)
	{
		WorkflowProcesses::Activity* activity = (WorkflowProcesses::Activity*)vActivities.at(i);
		activity->setProcessId ( process.getProcessId() );
		this->executeSQLCommands (connection, &(activity->getAttributes()), bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, false);
	}
}
...
void DataBaseTemplate::updateAttributes(WorkflowRelevantData::Attributes& vAttributes)
{
	...
	/*
	this->executeSQLCommands (connection, &vAttributes, ERASE, NULL, false);
	this->executeSQLCommands (connection, &vAttributes, ADD, NULL, true);
	*/
	...
	this->executeRecursiveSQLCommands(connection, &vAttributes2Search,bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL,false);
	...
	this->executeSQLCommands (connection, &vAttributes2Update, bOS::SQL::SQLCommand::SQLOperation::MODIFY, NULL, false);
	this->executeSQLCommands(connection, &vAttributes2Add, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);

	//this->executePrepareSQLCommands(connection, &vAttributes2Add, ADD, NULL, true);
...
void DataBaseTemplate::updateAttributes(DataManager::Panorama::WorkflowProcesses::Activity& activity)
{
	...
	this->executeSQL(connection, null, ERASE, acSQL, false);
...
void DataBaseTemplate::updateActivityCheckStatus(DataManager::Panorama::WorkflowProcesses::Activity* activity, string sStatus2Check)
{
	...
	this->executeSQLCommands (connection, activity, bOS::SQL::SQLCommand::SQLOperation::MODIFY, sWhere.getData(), true);
}
...
void DataBaseTemplate::updateActivity(DataManager::Panorama::WorkflowProcesses::Activity* activity, bool bRecursive)
{
	...
	if ( bRecursive )
	{
		this->executeRecursiveSQLCommands (connection, activity, bOS::SQL::SQLCommand::SQLOperation::MODIFY, NULL, true);
	}
	else
	{
		this->executeSQLCommands (connection, activity, bOS::SQL::SQLCommand::SQLOperation::MODIFY, NULL, true);
	}
}
...
void DataBaseTemplate::getAttributes(DataManager::Panorama::WorkflowRelevantData::Attributes& vAttributes)
{
	...
	this->executeRecursiveSQLCommands (NULL, &vAttributes, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
}
...
void DataBaseTemplate::getActivity(DataManager::Panorama::WorkflowProcesses::Activity& activity)
{
	...
	this->executeRecursiveSQLCommands (NULL, &activity, bOS::SQL::SQLCommand::SQLOperation::SELECT, NULL, true);
}
...
{
	this->executeRecursiveSQLCommands(NULL, message, bOS::SQL::SQLCommand::SQLOperation::ADD, NULL, true);
}
...
String DataBaseTemplate::getForm(const char* acFormName)
{
	throw DBException("Da sviluppare");
}
