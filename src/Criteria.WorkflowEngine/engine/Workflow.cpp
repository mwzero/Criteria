...
int Workflow::explore(WorkflowProcesses::Activity* task)
{
	...
	//bAnchor indica che il workflow è stato instanziato dal task exit di un sottoprocesso. 
	if ( bAnchor)
	{
	...
	//Se sCode è valorizzato indica che il workflow è stato istanziato dall'esterno grazie 
	...
	*/
	...
	{
		/*
			Questo flag indica se l'esecuzione del workflow è terminata.

			workflow termina:
				Eseguito un task END
				Eseguito un task CATCH(di tipo Sync) in sub-workflow.
				Eseguito un task CATCH(di tipo Only_One) in sub-workflow ed il task è stato già eseguito.
		*/
		...
		else if ( task->isCatcherType()  )
		{
			...
			case WorkflowProcesses::Activity::ONLY_ONE: 
				//task già eseguito quindi il primo workflow è gia passato da questo branch.
					...
					else
					{
						...
						case WorkflowProcesses::Activity::NEVER: //sync

							{
								...
								if ( task->isAlreadyExecuted() ) 
								{
									//se il task di split è già stato eseguito significa che uno dei workflow
									//generati dallo split non termina in un task di merge ma nello split
									//per cui nel caso che il reference task sia di tipo sync non è necessario
